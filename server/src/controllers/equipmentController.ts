...
export const addEquipment = async (req: Request, res: Response) => {
  ...
  try {
    const newEquipment: IFetchedEquipment = await addEquipmentToDatabase({
      name,
      areaId,
      isWorking,
    });
...
export const getAllEquipments = async (req: Request, res: Response) => {
  ...
  const isWorking =
    isWorkingParam === 'true'
      ? true
      : isWorkingParam === 'false'
      ? false
      : undefined;

  try {
    const equipment: IFetchedEquipment[] = await getAllEquipmentsFromDatabase(
      isWorking
    );
...
export const updateEquipment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(200).json('Equipment was updated successfully!');
  } catch (err) {
    res.status(500).json({ error: (err as Error).message });
    throw err;
  }
...
export const deleteEquipment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(200).json('Equipment was deleted successfully!');
  } catch (err) {
    res.status(500).json({ error: (err as Error).message });
    throw err;
  }
