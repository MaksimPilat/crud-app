...
export const addInspection = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(200).json(newInspection);
  } catch (err) {
...
export const getEquipmentInspections = async (req: Request, res: Response) => {
  ...
  try {
    const inspections: IFetchedInspection[] =
      await getInspectionsByEquipmentIdsFromDatabase([id]);
...
export const updateInspection = async (req: Request, res: Response) => {
  ...
  const { date, employeeId, result, causeOfFailure }: Omit<IInspection, 'id'> =
    req.body;

  try {
    await updateInspectionInDatabase({
      id,
      date,
      employeeId,
      result,
    ...
    return res.status(200).json('Inspection was updated successfully!');
  } catch (err) {
    res.status(500).json({ error: (err as Error).message });
    throw err;
  }
...
export const deleteInspection = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(200).json('Inspection was deleted successfully!');
  } catch (err) {
    res.status(500).json({ error: (err as Error).message });
    throw err;
  }
